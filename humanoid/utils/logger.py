...
class Logger:
    ...
    def _plot(self):
        ...
        #画出位置跟踪曲线
        a = axs[0, 0]
        if log["dof_pos0"]: a.plot(time, log["dof_pos0"], label='measured')
        if log["dof_pos_target0"]: a.plot(time, log["dof_pos_target0"], label='target')
        a.set(xlabel='time [s]', ylabel='Position [rad]', title='DOF Position 0')
        a.legend()
        a = axs[0, 1]
        if log["dof_pos1"]: a.plot(time, log["dof_pos1"], label='measured')
        if log["dof_pos_target1"]: a.plot(time, log["dof_pos_target1"], label='target')
        a.set(xlabel='time [s]', ylabel='Position [rad]', title='DOF Position 1')
        a.legend()
        a = axs[0, 2]
        if log["dof_pos2"]: a.plot(time, log["dof_pos2"], label='measured')
        if log["dof_pos_target2"]: a.plot(time, log["dof_pos_target2"], label='target')
        a.set(xlabel='time [s]', ylabel='Position [rad]', title='DOF Position 2')
        a.legend()
        a = axs[1, 0]
        if log["dof_pos3"]: a.plot(time, log["dof_pos3"], label='measured')
        if log["dof_pos_target3"]: a.plot(time, log["dof_pos_target3"], label='target')
        a.set(xlabel='time [s]', ylabel='Position [rad]', title='DOF Position 3')
        a.legend()
        a = axs[1, 1]
        if log["dof_pos4"]: a.plot(time, log["dof_pos4"], label='measured')
        if log["dof_pos_target4"]: a.plot(time, log["dof_pos_target4"], label='target')
        a.set(xlabel='time [s]', ylabel='Position [rad]', title='DOF Position 4')
        a.legend()
        a = axs[1, 2]
        if log["dof_pos5"]: a.plot(time, log["dof_pos5"], label='measured')
        if log["dof_pos_target5"]: a.plot(time, log["dof_pos_target5"], label='target')
        a.set(xlabel='time [s]', ylabel='Position [rad]', title='DOF Position 5')
        a.legend()
        a = axs[2, 0]
        if log["dof_pos6"]: a.plot(time, log["dof_pos6"], label='measured')
        if log["dof_pos_target6"]: a.plot(time, log["dof_pos_target6"], label='target')
        a.set(xlabel='time [s]', ylabel='Position [rad]', title='DOF Position 6')
        a.legend()
        a = axs[2, 1]
        if log["dof_pos7"]: a.plot(time, log["dof_pos7"], label='measured')
        if log["dof_pos_target7"]: a.plot(time, log["dof_pos_target7"], label='target')
        a.set(xlabel='time [s]', ylabel='Position [rad]', title='DOF Position 7')
        a.legend()
        a = axs[2, 2]
        if log["dof_pos8"]: a.plot(time, log["dof_pos8"], label='measured')
        if log["dof_pos_target8"]: a.plot(time, log["dof_pos_target8"], label='target')
        a.set(xlabel='time [s]', ylabel='Position [rad]', title='DOF Position 8')
        a.legend()
        a = axs[3, 0]
        if log["dof_pos9"]: a.plot(time, log["dof_pos9"], label='measured')
        if log["dof_pos_target9"]: a.plot(time, log["dof_pos_target9"], label='target')
        a.set(xlabel='time [s]', ylabel='Position [rad]', title='DOF Position 9')
        a.legend()
        a = axs[3, 1]
        if log["dof_pos10"]: a.plot(time, log["dof_pos10"], label='measured')
        if log["dof_pos_target10"]: a.plot(time, log["dof_pos_target10"], label='target')
        a.set(xlabel='time [s]', ylabel='Position [rad]', title='DOF Position 10')
        a.legend()
        a = axs[3, 2]
        if log["dof_pos11"]: a.plot(time, log["dof_pos11"], label='measured')
        if log["dof_pos_target11"]: a.plot(time, log["dof_pos_target11"], label='target')
